...
class DAG(object):
    ...
    def all_predecessors(self, nodes=[], predecessor=None):
        """ Returns a recursive list of all predecessors of the given list of predecessors """
        predecessor = predecessor or set([])
        for node in nodes:
            predecessor.add(node)
            self.all_predecessors(self.predecessors(node), predecessor)
        return predecessor
    ...
    def all_downstreams(self, node, graph=None):
        ...
        while i < len(nodes):
            ...
            i += 1
        return [node for node in self.topological_sort(graph=graph) if node in nodes_seen]
